...
::: incremental
::: {style="font-size: 0.65em;"}
-   Functional Programming (`base` R, `purrr`)
-   An introduction to Parallelism with `furrr`
-   Practice Questions
:::
:::
...
-   If $N_{(t + 1)} = N_{t}e^{r(1 - \frac{N_{t}}{K})}$, what is *N* for *t* = 4 ($N_{1} = 2$, *r* = 0.5, and *K* = 30)?

## Now we will do the math in R.

```{webR}
# First let's make some data to play with.
vec_dat <- data.frame(x = 1:5,
                      y = NA_real_)
vec_dat

loop_dat <- data.frame(t = 1:5,
                       N = NA_real_)
loop_dat
...
## Now we will look at a couple of the apply functions.
...
df <- lapply(paste0(getwd(), "/",
                    list.files(getwd(),
                               pattern = "WeatherStation_")),
             read.csv)
...
genetic_data <- data.frame(matrix(data = sample(c("A", "T", "G", "C"),
                                                3000,
                                                replace = TRUE),
                                  nrow = 300,
                                  ncol = 10))
...
## Now we will look at logical statements.
...
# Make some data.
df2 <- data.frame(ID = paste0("DataLogger-", i),
                    Temperature = runif(30, 10, 20),
                    Wind = runif(30, 0, 15),
                    Battery = "Normal")
...
for(i in seq_along(list.files(getwd(), pattern = "WeatherStation_"))) {
  tmp <- read.csv(paste0(getwd(), "/",
                         list.files(getwd(),
                                    pattern = "WeatherStation_"))[i])
  if(i == 1) {
    weather_dat <- tmp
  } else {
    weather_dat <- rbind(weather_dat, tmp)
  }
}
```
:::
...
write.csv(data.frame(ID = "DataLogger-7",
                     Temperature = c(runif(28, 10, 20), NA, NA),
                     Wind = c(runif(28, 0, 15), NA, NA),
                     Battery = c(rep("Normal", 28), "Low", "Low")),
...
```{webr-r}
dat <- data.frame(A = rnorm(100, 10, 5),
                  B = rpois(100, 10),
                  C = rnorm(100, 50, 10),
                  X = runif(100, 0, 100),
                  Y = rbeta(100, 1, 2),
...
df2$Week <- ifelse(df2$X <= 7,
                   1,
                   ifelse(df2$X >= 8 & df2$X <= 14,
                          2,
                          ifelse(df2$X >= 15 & df2$X <= 21,
                                 3,
                                 ifelse(df2$X >= 22 & df2$X <= 28,
                                        4,
                                        NA))))
```
:::
...
# `r fontawesome::fa("triangle-exclamation", "white")` Functions Should Return Output {background-color="#447099"}
...
> Parallel processing is not a magic bullet, ultimately performance depends on Overhead of Parallelization, Task Granularity, and Algorithm efficiency
...
# `r fontawesome::fa("laptop-code", "white")` Lets try Some exercises {background-color="#447099"}
...
Create a function named halve_until_less_than_one that takes a single numeric argument and keeps halving it until it is less than 1, then returns the result.
...
Write a function named `scale_columns` that takes a matrix and scales (normalizes) each column to have a mean of 0 and a standard deviation of 1.
...
```{webr-r}
library(purrr)
calculate_means <- function(list_of_vectors) {
  map(list_of_vectors, mean)
}
```
:::
