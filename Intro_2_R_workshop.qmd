...
author:
  ...
  - "Seth Rankins"
institute: 
  - "University Of Wyoming <br>Statistics"
...
<!--in order for chrome to load MathJax-->
...
df <- lapply(paste0(getwd(), "/",list.files(getwd(),pattern = "WeatherStation_")),
             read.csv)

lapply(df, function(x) head(x,3))[1:2] # first 2 of 20 #D? save on space

df2 <- do.call(rbind, lapply(paste0(getwd(), "/",list.files(getwd(),
                              pattern = "WeatherStation_")),read.csv))
head(df2)
```
...
## Now we will look at logical statements (1).

```{webr-r}
if(TRUE) {print("I have to work today.")} # save on space
...
## Now we will look at logical statements (2).
...
# Make some data.
df2 <- data.frame(ID = paste0("DataLogger-", 20), #D? repalce `i` with 20
                    Temperature = runif(30, 10, 20),
                    Wind = runif(30, 0, 15),
                    Battery = "Normal")
...
> Like a grammatically correct sentance, functions require a function name, argument and a returned object
