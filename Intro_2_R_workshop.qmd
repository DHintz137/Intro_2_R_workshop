...
- While loops are a good skill to know for people that write their own algorithms ^[Their usage, however, can be more niche as they are used when the number of iterations is not known a priori and reach completion via convergence criterion such as the max number of iterations or some metric like Euclidean Distance] 
...
formals(median) |> names() # defined with "..."
formals(mean) |> names()   # defined with "..."
formals(matrix) |> names() # NOT defined with "..."

f1 <- function(x, ...){
  out <- list(mean(x, ...), median(x, ...))
  out
}
f1(x=c(1:8,NA), na.rm = TRUE)
f1(x=c(1:8,NA), random_message = "Save the bees")
f2 <- function(x, ...){
  out <- list(mean(x, ...), median(x, ...), matrix(x,...))
  out
}
f2(x=c(1:8,NA), na.rm = TRUE)
```
...
# [Wanna Hear From Us?]{style="color:white; font-size: 70px;"} {background-color="#447099"}
